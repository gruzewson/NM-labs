function [country, source, degrees, x_coarse, x_fine, y_original, y_yearly, y_approximation, mse, msek] = zadanie5(energy)
    ...
    for i = 1:length(degrees)
        plot(x_fine, y_approximation{degrees(i)}, colors(i), 'DisplayName', ['Rząd ', num2str(degrees(i))]);
    end
    hold off;
    xlabel('x');
    ...
    % Drugi wykres: wartości błędów średniokwadratowych w funkcji stopnia aproksymacji cosinusowej
    subplot(3, 1, 2);
    semilogy(1:N, mse, '-o');
    ...
    % Trzeci wykres: wartości błędu różnicowego w funkcji rzędu aproksymacji cosinusowej
    subplot(3, 1, 3);
